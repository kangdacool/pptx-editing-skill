...
def ptable(sl, rows, l, t, w, widths, header_color, alt_color, white, ink, accent=None, size=24,
           rowh=1.9, title=None, font="Arial", min_pt=DEFAULT_MIN_PT):
    ...
    sc = w / sum(widths)
    gt = sl.shapes.add_table(nrow, ncol, Cm(l), Cm(t), Cm(w), Cm(rowh * nrow))
    tblx = gt.table
    for j, ww in enumerate(widths):
        tblx.columns[j].width = Cm(ww * sc)
    off = 0
    if title:
        tblx.rows[0].height = Cm(rowh)
        c0 = tblx.cell(0, 0)
    ...
    for i, rr in enumerate(rows):
        tblx.rows[i + off].height = Cm(rowh)
        head = (i == 0)
